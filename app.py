...
def result():
    score = 0
    total = 0
    print("Received form data:", request.form)  # Log de toutes les données reçues
    ...
    for key, value in request.form.items():
        if key.startswith('type'):
            ...
            answer = request.form.get(f'answer{index}', '').lower()  # Conversion en minuscule
            
            print(f"Comparing: expected '{expected}' to answer '{answer}'")  # Log de la comparaison
            
            if expected == answer:
                score += 1
            total += 1

    print(f"Final score: {score}/{total}")  # Log du score final
